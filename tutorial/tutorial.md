...
### Überblick: Was das Script tut

`main.js` ist für die gesamte Interaktivität der Editionsansicht zuständig:
1. Lädt die TEI-XML-Datei vom Server
2. Baut eine Liste aller 29 Seiten auf
3. Wandelt den XML-Inhalt der aktuellen Seite in HTML um
4. Reagiert auf Klicks (Vor/Zurück, Toggle)
5. Zeigt beim Hovern einen Tooltip mit der Rechenart an

### `'use strict'`
```javascript
'use strict';
```
Aktiviert den strikten Modus von JavaScript. Verhindert häufige Fehler,
z.B. das versehentliche Erstellen globaler Variablen. Gute Praxis für
jeden modernen JavaScript-Code.
...
let pages       = [];
let currentPage = 0;
let currentMode = 'diplomatic';
```
Der „State" sind alle Variablen, die sich während der Benutzung ändern
können. `xmlDoc` enthält das geparste XML-Dokument. `pages` ist eine
Liste aller Seiten mit Folio-Nummer und Bildpfad. `currentPage` und
`currentMode` merken sich, was gerade angezeigt wird.

### `async function init()` — Laden der XML-Datei
```javascript
async function init() {
...
```
`fetch()` lädt eine Datei vom Server — hier die XML-Datei. Das Wort
`async` und `await` bedeutet: „Warte, bis die Datei geladen ist, dann
mach weiter." Ohne `await` würde der Code weiterlaufen, bevor die Datei
fertig ist — und `xmlDoc` wäre noch leer.

`DOMParser` ist ein eingebautes Browser-Objekt, das einen XML-Text in
einen durchsuchbaren Baum (das DOM) umwandelt — genau wie der Browser
HTML parst, aber für XML.

**Warum fetch() einen lokalen Webserver braucht:** Browser erlauben aus
Sicherheitsgründen nicht, dass JavaScript lokale Dateien von der
Festplatte liest, wenn die Seite selbst auch lokal geöffnet ist
(`file://`). Ein lokaler Webserver (z.B. `python3 -m http.server`)
umgeht das, weil die Seite dann über `http://localhost` läuft.

### `buildPageList()` — Seitenliste aufbauen
```javascript
const surfaces = xmlDoc.getElementsByTagNameNS('*', 'surface');
// ...
const pbs = xmlDoc.getElementsByTagNameNS('*', 'pb');
```
`getElementsByTagNameNS('*', 'pb')` sucht alle `<pb>`-Elemente im
XML-Dokument. Das `'*'` steht für „egal welcher XML-Namespace" — nötig,
weil das TEI-XML einen eigenen Namespace hat.

Für jede `<pb>` wird gespeichert: die Folio-Nummer (`@n`), der Verweis
auf das Bild (`@facs`), und das Element selbst. Das Bild-URL wird aus
dem `<surface>`/`<graphic>`-Paar im XML-Header zusammengesetzt.

### `renderPageContent()` — XML in HTML umwandeln

Das ist die komplexeste Funktion. Sie geht den gesamten XML-Baum durch
und sammelt dabei, welche Knoten zur aktuellen Seite gehören.

```javascript
function walk(xmlNode) {
    if (xmlNode.localName === 'pb') {
        currentPageIdx++;   // neue Seite beginnt
        return;
    }
    if (xmlNode.localName === 'div') {
        // div immer traversieren — kann <pb> enthalten
        const htmlDiv = document.createElement('div');
        divStack.push(htmlDiv);
        for (const child of xmlNode.childNodes) walk(child);
        divStack.pop();
        if (htmlDiv.hasChildNodes()) { ... } // nur anhängen wenn Inhalt
        return;
    }
    if (currentPageIdx !== targetPage) {
        // Nicht auf der Zielseite: Kinder trotzdem durchsuchen
        for (const child of xmlNode.childNodes) walk(child);
        return;
    }
    // Auf der Zielseite: Element rendern
    ...
}
```

**Warum `<div>` immer traversieren?** Im TEI-XML können `<pb>`-Elemente
mitten in einem `<div>` stehen. Würden wir `<div>`s überspringen, wenn
wir noch nicht auf der Zielseite sind, würden wir die `<pb>`-Elemente
darin nicht zählen — und alle folgenden Seiten wären falsch.

**Der `divStack`** hält die aktuell offenen HTML-`<div>`-Elemente.
Wenn wir ein XML-`<div>` betreten, legen wir ein neues HTML-`<div>` oben
drauf. Wenn wir es verlassen, nehmen wir es wieder runter. So wird die
Verschachtelung der XML-Struktur in HTML übertragen.

### `renderElement()` — einzelne XML-Elemente
...
Für `<choice>` wird je nach Modus entweder `<abbr>` (diplomatisch) oder
`<expan>` (normalisiert) ausgewählt und gerendert. Die jeweils andere
Variante wird vollständig ignoriert.
...
`<g>`-Elemente enthalten mittelalterliche Sonderzeichen als Unicode-Zeichen
direkt im Textinhalt. `textContent` gibt diesen Text zurück, und wir
erstellen daraus einen einfachen Textknoten.
...
`addEventListener` hängt eine Funktion an ein Ereignis (hier: Klick).
Wenn der Button geklickt wird: Modus setzen, `active`-Klasse wechseln
(damit das CSS den richtigen Button einfärbt), Seite neu rendern.
...
function getTooltipInfo(element) {
    ...
    while (node && node !== transcriptionPanel) {
        ...
        if (type && SECTION_TYPES.has(type)) sectionType = type;
        else if (type) calcType = type;
        node = node.parentElement;
    }
}
```

Die Funktion geht vom gehoverten Element nach oben durch alle Vorfahren
(`parentElement`), bis sie das Transkriptions-Panel erreicht. Dabei
sammelt sie: den Abschnittstyp (`outline`/`reckoning_example`) und die
Rechenart (alles andere). So funktioniert der Tooltip auch bei
verschachtelten `<div>`-Elementen korrekt.
...
`event.clientX` und `event.clientY` sind die Mauskoordinaten im
Browserfenster. Der Tooltip wird 12 Pixel rechts und unterhalb der Maus
positioniert, damit er den Text darunter nicht verdeckt.
